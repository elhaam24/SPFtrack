...
if (empty($_SESSION['student_login'])) {
    header('location:' . (strpos($_SERVER['PHP_SELF'], '/student/') !== false ? '' : 'student/') . 'index.php');
...
// Fetch full student record (available to all pages via $student)
$student_q = mysqli_query($conn,
    "SELECT s.*, c.course_name, ss.session_name
     FROM student s
     LEFT JOIN course c ON s.course_id = c.course_id
     LEFT JOIN session ss ON s.session_id = ss.session_id
     WHERE s.student_id = $student_id");
$student = mysqli_fetch_assoc($student_q);

// Unread notification count for bell
$bell_count = 0;
$bell_q = mysqli_query($conn, "SELECT COUNT(*) AS c FROM notification WHERE student_id=$student_id AND notification_status='unread'");
if ($bell_q) { $bell_row = mysqli_fetch_assoc($bell_q); $bell_count = $bell_row['c'] ?? 0; }

// Unread message count
$msg_count = 0;
$msg_q = mysqli_query($conn, "SELECT COUNT(*) AS c FROM messages WHERE receiver_id=$student_id AND receiver_type='student' AND is_read=0");
...
<head>
    ...
    <title>SPF Track – Student Portal</title>
...
<aside id="sidebar-drawer" class="fixed top-0 left-0 h-full w-72 bg-surface z-50 shadow-level-2 transform -translate-x-full flex flex-col">
    ...
    <div class="px-6 py-4 bg-surface-container-low border-b border-outline-variant">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="font-body-md font-semibold text-on-surface"><?php echo $student_name; ?></p>
                <p class="font-body-sm text-on-surface-variant">Student</p>
            </div>
        </div>
    </div>
    <!-- Nav links -->
    <nav class="flex-1 overflow-y-auto py-4 px-3">
        ...
        $nav_items = [
            ...
            ['icon'=>'school',           'label'=>'Course Units',      'href'=>'CourseUnits.php'],
            ['icon'=>'assignment',       'label'=>'Assignments',       'href'=>'view_assignments.php'],
            ['icon'=>'upload_file',      'label'=>'Submit Assignment', 'href'=>'submit_assignments.php'],
            ['icon'=>'task_alt',         'label'=>'Submission Status', 'href'=>'submission_status.php'],
            ['icon'=>'menu_book',        'label'=>'Materials',         'href'=>'materials.php'],
            ['icon'=>'analytics',        'label'=>'Progress Report',   'href'=>'progressReport.php'],
            ['icon'=>'grade',            'label'=>'Results',           'href'=>'result.php'],
...
<nav class="md:hidden fixed bottom-0 left-0 w-full flex justify-around items-center px-4 py-2 bg-inverse-surface shadow-lg z-40">
    ...
    $bottom_nav = [
        ...
        ['icon'=>'school',       'label'=>'Courses',  'href'=>'CourseUnits.php'],
        ['icon'=>'chat_bubble',  'label'=>'Messages', 'href'=>'messages.php'],
        ['icon'=>'analytics',    'label'=>'Reports',  'href'=>'progressReport.php'],
        ['icon'=>'notifications','label'=>'Alerts',   'href'=>'notification.php'],
    ];
